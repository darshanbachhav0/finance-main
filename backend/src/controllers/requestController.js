...
export const registerInvoice = asyncHandler(async (req, res) => {
  ...
  res.json({ data: publicRequestPayload(result.request), sunatVoucher: result.sunatVoucher, accountsPayable: result.accountsPayable, observed: result.observed });
});
...
export const settleRenditionBalance = asyncHandler(async (req, res) => {
  ...
  res.json({ data: publicRequestPayload(result.request), journal: result.journal });
});
