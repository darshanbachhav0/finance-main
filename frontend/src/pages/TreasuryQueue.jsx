...
import PaymentCalendar from "../components/PaymentCalendar.jsx";
...
export default function TreasuryQueue({ historyOnly = false }) {
  ...
  return <section>
    <WorkspaceTools links={[["Reimbursement Banking", "/reimbursement-bank"], ["Suppliers", "/suppliers"], ["Management Reports", "/reports"]]} />
      <PageHeader title={historyOnly ? "Payment History" : "Payments"} description="Schedule and confirm payments." actions={<><PaymentCalendar /><button type="button" className="secondary-button" onClick={reloadAll} disabled={loading}><RefreshCw className={loading ? "spin" : ""} size={16} /><span>{t("Refresh")}</span></button></>} />
