...
export default function DraftPanel({ draft, onDiscard, children, busy = false }) {
  ...
  return <>
    <div className={`work-draft-status ${["error", "load-error", "conflict"].includes(draft.status) ? "work-draft-warning" : ""}`}>
      <div role="status"><strong>{t(labels[draft.status] || labels.ready)}</strong><small>{t("Private draft Â· Closing keeps your progress Â· Submission is still required")}</small>
