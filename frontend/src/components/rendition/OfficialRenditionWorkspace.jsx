...
function ReadOnlyRows({ rows, type }) {
  ...
  return <div className="rendition-record-list">{rows.map((row, index) => <div key={row._id || index} className={row.limitExceeded ? "has-warning" : ""}>
    ...
    <b>{formatCurrency(type === "mobility" ? row.amount : row.grossAmount, "PEN", language)}</b>
  </div>)}</div>;
}
...
export default function OfficialRenditionWorkspace({ request, masters, user, onReload }) {
  ...
  return <div className="workspace-panel detail-section official-rendition-workspace">
    ...
    <Section icon={MapPin} title="Local Transportation" description="Repeatable PEN mobility details; the daily configuration is a warning unless Finance configures blocking behavior." action={editable && <button type="button" className="secondary-button compact-button" onClick={() => setMobilityLines((rows) => [...rows, emptyMobility()])}><Plus size={15} />{t("Add row")}</button>}>
      ...
      <div className="rendition-section-footer"><span>{t("Mobility Subtotal")}</span><strong>{formatCurrency(editable ? mobilitySubtotal : request.rendition?.mobilitySubtotal || 0, "PEN", language)}</strong></div>
      {policy.mobility && <div className={`inline-alert ${warningDates.length || request.rendition?.limitEvaluation?.exceededLineCount ? "alert-warning" : "alert-info"}`}><AlertTriangle size={17} /><div><strong>{t("Daily Mobility Limit")}: {formatCurrency(policy.mobility.numericValue, "PEN", language)}</strong><span>{t(warningDates.length || request.rendition?.limitEvaluation?.exceededLineCount ? "One or more days exceed the configured warning value. Submission is not automatically rejected." : "Current entries are within the effective configured value.")}</span></div></div>}
    </Section>
    ...
    <Section icon={ReceiptText} title="Expenses Without Supporting Documents" description="Exceptional lines require a detailed description and do not fabricate fiscal evidence." action={editable && <button type="button" className="secondary-button compact-button" onClick={() => setUnsupportedLines((rows) => [...rows, emptyUnsupported()])}><Plus size={15} />{t("Add row")}</button>}>
      ...
      <div className="rendition-section-footer"><span>{t("Unsupported Expense Subtotal")}</span><strong>{formatCurrency(editable ? unsupportedSubtotal : request.rendition?.unsupportedExpenseSubtotal || 0, "PEN", language)}</strong></div>
    ...
    <Section icon={Banknote} title="Totals / Reconciliation" description="The server recalculates all official totals and never trusts browser-submitted subtotals.">
      <div className="rendition-totals"><div><span>{t("Mobility Subtotal")}</span><strong>{formatCurrency(editable ? mobilitySubtotal : request.rendition?.mobilitySubtotal || 0, "PEN", language)}</strong></div><div><span>{t("Unsupported Expense Subtotal")}</span><strong>{formatCurrency(editable ? unsupportedSubtotal : request.rendition?.unsupportedExpenseSubtotal || 0, "PEN", language)}</strong></div><div><span>{t("Total to Reimburse")}</span><strong>{formatCurrency(editable ? officialTotal : request.rendition?.reimbursementTotal || 0, "PEN", language)}</strong></div><div><span>{t("Accounting Amount")}</span><strong>{formatCurrency(editable ? accountingAmount : request.rendition?.detailReconciliation?.accountingRenderedAmount || request.rendition?.amountRendered || 0, "PEN", language)}</strong></div><div><span>{t("Difference")}</span><strong className={Math.abs(editable ? difference : request.rendition?.detailReconciliation?.difference || 0) < 0.01 ? "text-success" : "text-warning"}>{formatCurrency(editable ? difference : request.rendition?.detailReconciliation?.difference || 0, "PEN", language)}</strong></div><div><span>{t("Reconciliation")}</span><StatusBadge status={editable ? (Math.abs(difference) < 0.01 ? "MATCH" : "MISMATCH") : request.rendition?.detailReconciliation?.status} /></div></div>
      {isAdvance && <div className="advance-reconciliation"><span>{t("Amount advanced")}: {formatCurrency(advancedAmount, "PEN", language)}</span>{editable && <label className="field"><span>{t("Amount returned")}</span><input type="number" min="0" step="0.01" value={amountReturned} onChange={(event) => setAmountReturned(event.target.value)} /></label>}<span>{t("Outstanding balance")}: <strong className={Math.abs(balance) < 0.01 ? "text-success" : "text-warning"}>{formatCurrency(editable ? balance : request.rendition?.balanceOutstanding || 0, "PEN", language)}</strong></span></div>}
    </Section>
    ...
    {isAdvance && request.rendition?.status === "VALIDATED" && <Section icon={Landmark} title="Account 14 Regularization" description="Non-deductible amounts remain charged to the collaborator until reimbursement or payroll deduction is recorded.">
      <div className="non-deductible-summary"><div><span>{t("Non-deductible outstanding")}</span><strong className={nonDeductibleOutstanding > 0 ? "text-warning" : "text-success"}>{formatCurrency(nonDeductibleOutstanding, "PEN", language)}</strong></div><div><span>{t("Settlement status")}</span><StatusBadge status={nonDeductibleOutstanding > 0 ? "PENDING" : "RESOLVED"} /></div></div>
      {(request.rendition?.nonDeductibleSettlements || []).length > 0 && <div className="compact-lines">{request.rendition.nonDeductibleSettlements.map((item, index) => <div key={item._id || `${item.method}-${item.settledAt}-${index}`}><span>{formatDateTime(item.settledAt)} · {t(item.method)} · {item.reference}</span><strong>{formatCurrency(item.amount || 0, "PEN", language)}</strong></div>)}</div>}
    ...
    <Section icon={FileSignature} title="Beneficiary Acknowledgment" description="Authenticated electronic acknowledgment; this is not described as a certified digital signature.">
      {editable ? <label className="checkbox-row acknowledgment"><input type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} /><span><strong>{t("I acknowledge the accuracy of this rendition and submit it using my authenticated account.")}</strong><small>{user.name} · {user.email}</small></span></label> : <dl className="detail-grid"><div><dt>{t("Signer")}</dt><dd>{request.rendition?.beneficiaryAcknowledgment?.signerName || "-"}</dd></div><div><dt>{t("Signed")}</dt><dd>{request.rendition?.beneficiaryAcknowledgment?.signedAt ? formatDateTime(request.rendition.beneficiaryAcknowledgment.signedAt) : "-"}</dd></div><div><dt>{t("Reference")}</dt><dd className="mono-reference">{request.rendition?.beneficiaryAcknowledgment?.reference || "-"}</dd></div></dl>}
    </Section>
