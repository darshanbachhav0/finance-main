...
export default function Drawer({ open, title, description, size = "medium", children, footer, onClose }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      ...
      const items = [...drawerRef.current.querySelectorAll('summary,a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')].filter((item) => item.getClientRects().length && !item.closest('[inert]'));
