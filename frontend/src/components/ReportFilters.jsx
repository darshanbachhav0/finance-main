  ...
  return (
    <section className={`report-filter-shell${expanded ? " is-expanded" : ""}`} aria-label={t("Report filters")}>
      <button type="button" className="filter-toggle" onClick={() => setExpanded((current) => !current)} aria-expanded={expanded}>
        <Filter size={16} /><span>{t("More filters")}</span><ChevronDown size={16} />
      </button>
      <div className="report-filter-content focused-report-filters">
        <div className="report-filter-grid">
          ...
          <label hidden={!expanded} className="field"><span>{t("Cost center")}</span><select value={values.costCenter} onChange={update("costCenter")}><option value="">{t("All Cost Centers")}</option>{options.costCenters.map((item) => <option key={item.value} value={item.value}>{item.code} - {item.name}</option>)}</select></label>
          <label hidden={!expanded} className="field"><span>{t("Project")}</span><select value={values.project} onChange={update("project")}><option value="">{t("All projects")}</option>{options.projects.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        </div>
