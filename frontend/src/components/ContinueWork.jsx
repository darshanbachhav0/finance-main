...
export default function ContinueWork() {
  ...
  return <section className="workspace-panel continue-work"><h2>{t("Continue your work")}</h2><p>{t("Unfinished work saved privately to your account.")}</p>{error && <p role="alert">{error} <button type="button" onClick={load}>{t("Retry")}</button></p>}
    {(expanded ? rows : rows.slice(0, 5)).map(row => <div className="continue-work-row" key={row._id}><div><strong>{t(row.title)}</strong><small>{new Date(row.updatedAt).toLocaleString()}</small></div><div className="work-draft-actions"><Link className="secondary-button" to={`${row.route}?${new URLSearchParams({ workDraft: row._id, workScope: row.scope, workRecord: row.recordId })}`}>{t("Continue")}</Link><button type="button" className="text-button" onClick={() => setDiscarding(row)}>{t("Discard draft")}</button></div></div>)}
    ...
    {rows.length > 5 && <button type="button" className="text-button" onClick={() => setExpanded(!expanded)}>{t(expanded ? "Show less" : "Show all drafts")}</button>}
  </section>;
}
