...
export default function BudgetPlanWorkspace({ open, planId, year, selectedPeriod, canManage, onClose, onSaved }) {
  ...
  return <Drawer open={open} title={planId || plan ? "Annual budget plan" : "Create annual budget"} size="large" onClose={() => !saving && onClose()}>
    ...
    {loading ? <p role="status">{t("Loading...")}</p> : plan ? <>
      ...
      <div className="budget-month-cards">{plan.months.map(bucket => <article key={bucket.month}><strong>{t(BUDGET_MONTHS[bucket.month - 1])}</strong><span>{t("Available")}: {money(bucket.availableAmount)}</span><details><summary>{t("Details")}</summary><dl>{[["Assigned", plan.planningMode === "ANNUAL_ONLY" ? null : bucket.assignedAmount], ["Committed", bucket.committedAmount], ["Executed", bucket.executedAmount], ["Paid", bucket.paidAmount]].map(([label, value]) => <div key={label}><dt>{t(label)}</dt><dd>{money(value)}</dd></div>)}</dl></details></article>)}</div>
      <div className="budget-month-table"><table><thead><tr>{["Month", "Assigned", "Committed", "Executed", "Paid", "Available"].map((label) => <th key={label}>{t(label)}</th>)}</tr></thead><tbody>{plan.months.map((bucket) => <tr key={bucket.month} className={selectedPeriod === `${plan.period}-${String(bucket.month).padStart(2, "0")}` ? "is-current-month" : ""}><th scope="row">{t(BUDGET_MONTHS[bucket.month - 1])}</th><td data-label={t("Assigned")}>{plan.planningMode === "ANNUAL_ONLY" ? "—" : money(bucket.assignedAmount)}</td><td data-label={t("Committed")}>{money(bucket.committedAmount)}</td><td data-label={t("Executed")}>{money(bucket.executedAmount)}</td><td data-label={t("Paid")}>{money(bucket.paidAmount)}</td><td data-label={t("Available")} className={bucket.availableAmount < 0 ? "text-danger" : ""}>{money(bucket.availableAmount)}</td></tr>)}</tbody></table></div>
